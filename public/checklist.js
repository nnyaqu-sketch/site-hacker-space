async function loadLists(){
  const res = await fetch(apiPath('/api/checklists'));
  ...
  lists.forEach(l => {
    ...
    const addBtn = document.createElement('button'); addBtn.textContent='Add';
      ...
      await fetch(apiPath(`/api/checklist/${l.id}/items`), { method: 'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ text }) });
      loadLists();
    });
...
document.getElementById('create-list').addEventListener('click', async ()=>{
  ...
  await fetch(apiPath('/api/checklists'), { method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ name, description: desc }) });
...
document.addEventListener('change', async (e)=>{
  if (e.target.matches('input[type=checkbox][data-id]')){
    ...
    await fetch(apiPath(`/api/checklist/items/${id}/toggle`), { method:'POST' });
    loadLists();
  }
});
...
const so = io((window.API_BASE || '') + '/public');
