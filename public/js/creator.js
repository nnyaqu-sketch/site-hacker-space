...
async function loadStats() {
    try {
        const res = await fetch(apiPath('/api/creator/stats'));
...
async function loadUsers() {
    try {
        const res = await fetch(apiPath('/api/creator/users'));
...
function setupEventListeners() {
    ...
    if (clubOpenBtn) {
        clubOpenBtn.addEventListener('click', async () => {
            try {
                const res = await fetch(apiPath('/api/send-club-open'), {
                    method: 'POST',
    ...
    document.getElementById('generate-admin').addEventListener('click', async () => {
        try {
            const res = await fetch(apiPath('/api/creator/generate-admin-code'), { method: 'POST' });
    ...
    document.getElementById('set-message').addEventListener('click', async () => {
        ...
        try {
            await fetch(apiPath('/api/creator/system-message'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    document.getElementById('clear-chat').addEventListener('click', async () => {
        ...
        try {
            await fetch(apiPath('/api/creator/clear-chat'), { method: 'POST' });
    ...
    document.getElementById('clear-announcements').addEventListener('click', async () => {
        ...
        try {
            await fetch(apiPath('/api/creator/clear-announcements'), { method: 'POST' });
...
async function deleteUser(userId) {
    ...
    try {
        await fetch(apiPath(`/api/creator/users/${userId}`), { method: 'DELETE' });
        loadUsers();
    } catch (err) {
