...
// `API_BASE` can be set in `public/js/config.js` or inline on the page.
...
function socketNamespace(ns) {
  ...
  return (API_BASE ? API_BASE : '') + ns;
}

let me = { loggedIn: false };

async function checkAuth() {
  const r = await fetch(apiPath('/api/me'));
  me = await r.json();
  updateStatus();
...
async function loadSystemMessages() {
  const res = await fetch(apiPath('/api/system-messages'));
  if (!res.ok) return;
...
function setupAuth() {
  ...
  loginBtn?.addEventListener('click', async () => {
    ...
    const r = await fetch(apiPath('/api/login'), {
      method: 'POST',
...
try {
  const publicSocket = io(socketNamespace('/public'));
