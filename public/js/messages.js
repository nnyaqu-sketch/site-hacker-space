...
let allUsers = [];
const socket = io((window.API_BASE || '') + '/messages');
...
async function loadConversation(userId) {
    try {
        const res = await fetch(apiPath(`/api/messages/${userId}`));
...
async function sendMessage() {
    ...
    try {
        const res = await fetch(apiPath(`/api/messages/${currentConversationUser.id}`), {
            method: 'POST',
