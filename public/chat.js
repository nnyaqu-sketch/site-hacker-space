const socket = io((window.API_BASE || '') + '/public');
...
fetch(apiPath('/api/me')).then(r=>r.json()).then(j=>{ me = j; updateStatus(); });
...
document.getElementById('login-btn').addEventListener('click', async ()=>{
  ...
  const r = await fetch(apiPath('/api/login'), { method: 'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ username, password }) });
...
document.getElementById('register-btn').addEventListener('click', async ()=>{
  ...
  const r = await fetch(apiPath('/api/register'), { method: 'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ username, password, code }) });
  const j = await r.json();
